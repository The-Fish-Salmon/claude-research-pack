...
- **Phase 1**: 1× `scoping` agent (sequential).
- **Phase 2**: 3× `investigator` agents in parallel — one per top-level subtopic. Each may make several MCP calls.
- **Phase 3**: 1× `synthesizer` + 1× `bias-auditor` in parallel, sharing the Phase 2 output.
- **Phase 3.5**: 1× `devils-advocate` (checkpoint 1).
- **Phase 4**: 1× `composer` (sequential, large context).
- **Phase 5**: 1× `editor` + 1× `ethics` + 1× `devils-advocate` (checkpoint 2 & 3) in parallel.

For `quick`, run scoping → 1× investigator → 1× composer → 1× devils-advocate. Skip bias auditor.
