...
2. **Investigation** -- 2x `investigator.md` in parallel. Each pulls 8-15 candidate papers across the MCP priority order. Goal: 15-30 papers in the long list.
...
5. **Synthesis** -- 1x `synthesizer.md` only. (Bias auditor is lighter here -- fold into annotations.)
6. **Devil's Advocate** -- one checkpoint after synthesis. Can block if the synthesis overstates.
7. **Composition** -- 1x `composer.md`, against [templates/report_lit_review.md](../templates/report_lit_review.md).
