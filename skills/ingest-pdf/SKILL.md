---
name: ingest-pdf
description: Ingest a local PDF (or folder of PDFs) into the Obsidian vault. Extracts DOI/title, resolves metadata, writes 30_Literature/CITEKEY.md. Use /ingest-pdf or "ingest this PDF".
---
...
3. **For each PDF, extract metadata in this order:**
   - **(a) DOI sniff from text.** Use the `Read` tool (which can ingest PDFs
...
- It does NOT auto-tag the new note with a project. The user does that
  manually or via `lit-status` / project notes later.
